    ...
    @endif
      <form method="post" action="{{ route('item.update', $item->item_id) }}">
          ...
          <div class="form-group">
              ...
              <input type="number" step="0.01" class="form-control" name="list_price" value="{{ $item->list_price }}"/>
          </div>
          <div class="form-group">
              <label for="qtyInStock">Quantity in Stock</label>
              <input type="number" class="form-control" name="qtyInStock" value="{{ $item->qtyInStock }}"/>
          </div>
          <div class="form-group">
